			...
			var jqxhr = $.getJSON("abnina.php", function(data) {
                ...
                var messageDiv = $('#1_TampungListAgent'); 
					...
					$.each(data, function(i, item) {
							...
							if(object!== undefined){
								...
								$('#summaryCall').sparkline([jumlahNyaACD,jumlahNyaAbn,CallsWaiting],{ type:'pie', barColor:'green',width:'150',height:'150' });
								//$('#valPie').html("1,2,2");
                            }
                ...
                $.each(data.DataDetail, function (x, itemx) {
                    ...
                    result += '<a href="#" class="list-item">' +
                        ...
                        '<div style="font-size:15px; color:rgb(255, 255, 255);" class="list-text-info">' +
                        '<i class="icon-circle"></i> ' +
                        itemx['State'] + ' - ' + itemx['AUX Reason'] + ' - ' + hours+':'+minutes+':'+seconds +
                        '</div>' +
                        '</div>' +
                        '</a>';
                    messageDiv.empty();
...
function getData_AgentStates() {
    ...
    var divlblStaffed = $('#lblStaffed');
    var divlblAvail = $('#lblAvail');
    var divlblAux = $('#lblAux');
            ...
            console.log(errorThrown);
				...
				var jqxhr = $.getJSON("abnout.php", function (data) {
					...
					$.each(data, function(i, item) {
							...
							valueStaffed = item['Agents Staffed'][0];
							valueAvail = item['Agents Avail'][0];
							valueAux = item['Agents in AUX'][0];
							divlblStaffed.empty();
							divlblAvail.empty();
							divlblAux.empty();
							divlblStaffed.text(valueStaffed);
							divlblAvail.text(valueAux);
							divlblAux.text(valueAvail);
					  });
